...
Sphere SceneS[] = Sphere[] (
	...
	createSphere(vec3( 0, -1, 0),                                    1, vec3(1.0, 1.0, 1.0), vec3(1.0, 1.0, 1.0), vec3(0), 0.1, 0.1, 0.9, 1.5),
	createSphere(vec3( 2.5*cos( timei + PI / 2    ), -1,   2.5*sin( timei + PI / 2    )), 1, vec3(1.0, 0.2, 0.2), vec3(0.1, 1.0, 0.8), vec3(0), 0.0, 0.1, 0.9, 1.5),
	createSphere(vec3( 2.5*cos( timei + PI        ), -1.1, 2.5*sin( timei + PI        )), 1, vec3(0.2, 0.2, 1.0), vec3(1.0, 1.0, 0.8), vec3(0), 0.7, 1.0, 0.0, 1.5),
	createSphere(vec3( 2.5*cos( timei + PI * 3 / 2), -1.2, 2.5*sin( timei + PI * 3 / 2)), 1, vec3(0.2, 1.0, 0.2), vec3(1.0, 1.0, 1.0), vec3(0), 0.0, 0.5, 0.0, 1.5),
